...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // Insert ke database
  $query = "INSERT INTO tb_komentar (nama, komentar, parent_id, berita_id, tanggal_komen) VALUES (?, ?, ?, ?, ?)";
  $stmt = $db->prepare($query);
  if ($stmt === false) {
...
// lakukan query select order by untuk mengurutkan komentar baru pada tb_komentar
$query = "SELECT * FROM tb_komentar WHERE parent_id = 0 AND berita_id = ? ORDER BY id DESC";
$stmt = $db->prepare($query);
if ($stmt === false) {
...
{
  $output = '';
  $query = "SELECT * FROM tb_komentar WHERE parent_id=? ORDER BY id ASC";
  $stmt = $db->prepare($query);
  if ($stmt === false) {
    die("Prepare failed: " . $db->error);
  }
...
$query = "SELECT COUNT(*) as total_comments FROM tb_komentar WHERE berita_id = ?";
$stmt = $db->prepare($query);
if ($stmt === false) {
  die("Prepare failed: " . $db->error);
}
              ...
              <form action="" method="post">
                ...
                <div class="row">
                  <div class="col-md-12 form-group">
                    <input name="nama" id="nama" type="text" class="form-control" placeholder="Your Name*">
                  </div>
